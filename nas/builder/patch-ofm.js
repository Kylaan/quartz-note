...
// 4) KaTeX 不支持 \begin{eqnarray}（旧环境，会抛 "No such environment: eqnarray"）。
//    转成 KaTeX 支持的 align，并把三列对齐 &=& 改成两列 &=（代码块内保持原样）。
const eqnHelper = [
  "function __obsEqnarray(src) {",
  "  var parts = src.split(/(```[\\s\\S]*?```|~~~[\\s\\S]*?~~~)/g);",
  "  for (var i = 0; i < parts.length; i += 2) {",
  "    parts[i] = parts[i].replace(/\\\\begin\\{eqnarray(\\*?)\\}([\\s\\S]*?)\\\\end\\{eqnarray\\*?\\}/g, function (m, star, body) {",
  "      return '\\\\begin{align' + star + '}' + body.replace(/&([^&]*)&/g, '&$1') + '\\\\end{align' + star + '}';",
  "    });",
  "  }",
  "  return parts.join('');",
  "}",
...
if (!s.includes("__obsEqnarray")) {
  s = eqnHelper + "\n" + s;
  // 在 $$ 规范化之前先把 eqnarray 转成 align
  s = s.replace("textTransform(_ctx, src) {", () => "textTransform(_ctx, src) { src = __obsEqnarray(src);");
  done.push("eqnarray");
}

fs.writeFileSync(F, s);
for (const k of ["svg", "html", "math", "eqnarray"]) {
  if (!done.includes(k)) {
